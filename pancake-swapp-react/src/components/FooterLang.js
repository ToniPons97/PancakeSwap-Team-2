...
const FooterLang = () => {
    const footerLangsRef = useRef(null);
    const [ currentLang, setCurrentLang ] = useState('EN');

    const handleLangSwitch = (ev) => {
        if (ev.target.value) {
            setCurrentLang(prev => ev.target.value);
            footerLangsRef.current['style'].display = '';
            setTimeout(() => footerLangsRef.current.style['display'] = 'none', 0);
            setTimeout(() => footerLangsRef.current.style['display'] = '', 70);
        }
    }

    return (
        <div id="footer-lang">
            <div ref={footerLangsRef} id="lang-icons-wrapper">
                <ul onClick={handleLangSwitch} id="footer-langs">
                <button value="AR">
                    العربية
                </button>
                <button value="BN">
